...
if (isset($_POST['filter'])){
       ...
       $range2=$_POST['range2'];
            ...
            <div class="card-header py-3">
              ...
              <h4 class="m-2 font-weight-bold text-primary blocky">Stock Control Report FROM <?php echo $range1 ?> TO <?php echo $range2 ?></h4>
              <form class="blocky" method="post" action="">
              ...
              <a href="emp_sales.php"><button class="btn btn-info">Sales Report by Employee Details</button></a>
                                    <a href="printer-fill.php?date1=<?php echo $range1?>&date2=<?php echo $range2?>" class="btn btn-primary text-white" name="reverse">Print Report</a>
    ...
    $query = "SELECT*, PRODUCT_ID, PRODUCT_CODE, bcode,products_tbl.price, product.NAME, products_tbl.name, SUM(QTY_STOCK) AS QTY_STOCK,  DATE_STOCK_IN,EXPIRY FROM product inner join products_tbl on product.PRODUCT_CODE=products_tbl.pcode inner join category on products_tbl.cat=category.CATEGORY_ID WHERE product.STATUS=1 GROUP BY PRODUCT_CODE  order by QTY_STOCK DESC ";
      ...
      $sn=1;
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                <!-- opening stock -->
                <td><?php 
$query5 = "(SELECT*, SUM(QTY_STOCK) AS zigi  from product WHERE product.STATUS=1 AND tdate between '$range1' AND '$range2' AND STATUS=1 AND NATURE='Purchase' AND PRODUCT_CODE='$pcode' )
              ";
...
$query12 = "(SELECT*, SUM(QTY_STOCK) AS zigi  from product WHERE product.STATUS=1 AND tdate between '$range1' AND '$range2' AND STATUS=1 AND NATURE='Sales' AND PRODUCT_CODE='$pcode' )
              ";
...
$query120 = "(SELECT*, SUM(QTY_STOCK) AS zigi  from product WHERE product.STATUS=1 AND tdate between '$range1' AND '$range2' AND STATUS=1 AND NATURE='Adjustment' AND PRODUCT_CODE='$pcode' )
              ";
...
<?php
$query2 = "(SELECT*, SUM(QTY_STOCK) AS zigi  from product WHERE product.STATUS=1 AND tdate between '$range1' AND '$range2' AND STATUS=1 AND NATURE='Purchase' AND PRODUCT_CODE='$pcode' )
              ";
...
<!-- Balance module -->
               ...
               <td>
                 ...
                 <?php
$query12 = "(SELECT*, SUM(QTY_STOCK) AS zigi  from product WHERE product.STATUS=1 AND tdate between '$range1' AND '$range2' AND STATUS=1 AND NATURE='Adjustment' AND PRODUCT_CODE='$pcode' )
              ";
