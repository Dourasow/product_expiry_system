...
function RemoveSpecialChar($word)
{
    ...
    //Search query.
       $Query = "SELECT*, sum(QTY_STOCK) as rem  FROM products_tbl inner join product on products_tbl.pcode=product.PRODUCT_CODE WHERE product.STATUS=1 AND products_tbl.status='1'AND products_tbl.name LIKE '%$Name%' or bcode LIKE '$Name%' or pcode LIKE '%$Name'  GROUP BY products_tbl.pcode LIMIT 10 ";
    //Query execution
    ...
    <ul>
       ...
       <div class="col-sm-4 col-md-2 block" >
                                      <form method="post" action="bkpos.php?action=add&id=<?php echo $product['p_id']; ?>">
                                          <div class="products">

                                            
                                              <h6 class="text-info"><?php if ($product['type']==2) {
                                                        echo $product['name']."".'(-)';
                                                   }else{
                                              ...
                                              <h6>NGN <?php echo $product['price']; ?></h6>
                                                ...
                                                ?>
                                              <input type="number" name="quantity" class="form-control" value="<?php echo $qtyi ?>" onkeypress="return isNumberKey(event)" />  <input type="hidden" name="squantity" class="form-control" value="1" />  <input type="hidden" name="name" class="form-control" value="<?php echo $product['name']; ?>" />
